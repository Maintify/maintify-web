                                    ...
                                    <div class="inline-flex items-center gap-2 justify-end">
                                        ...
                                        @if($usr->id !== auth()->id())
                                            <form action="{{ route('admin.users.destroy', $usr->id) }}" method="POST"
                                                  onsubmit="return confirm('Apakah Anda yakin ingin MENGHAPUS permanen akun \'{{ $usr->name }}\'? Tindakan ini tidak dapat dibatalkan.');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="px-3 py-1.5 bg-rose-950/40 hover:bg-rose-900/60 border border-rose-900/50 text-rose-400 hover:text-rose-200 text-xs font-semibold rounded-lg transition-all flex items-center gap-1">
