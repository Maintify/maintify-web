<x-app-layout>
    @slot('pageTitle', 'Katalog Sparepart')
    @slot('breadcrumb', 'Workshop / Spareparts')
    ...
    <div class="max-w-6xl mx-auto px-4 py-8">
        ...
        <div class="bg-[#181A1A] border border-[#2E3030] rounded-2xl p-4 mb-6">
            <form method="GET" action="{{ route('workshop.spareparts.index') }}" class="flex gap-3">
                <div class="relative flex-1">
                    <input type="text"
                           name="search"
                           value="{{ $search }}"
                           placeholder="Cari sparepart berdasarkan nama atau kategori..."
                    ...
                    <tbody class="divide-y divide-[#2E3030]">
                        @forelse($spareparts as $part)
                            <tr class="hover:bg-zinc-900/30 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="font-semibold text-zinc-200">{{ $part->name }}</div>
                                </td>
                                <td class="px-6 py-4">
                                    @if($part->category)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-zinc-800 border border-zinc-700 text-zinc-300">
                                            {{ $part->category }}
                                        </span>
                                    @else
                                ...
                                <td class="px-6 py-4 text-center">
                                    ...
                                    @else
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-zinc-900 border border-zinc-800 text-zinc-500">
                                            <span class="w-1.5 h-1.5 rounded-full bg-zinc-650"></span>
                                            Non-Aktif
                                        </span>
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    ...
                                    <p class="text-zinc-400 text-sm font-semibold">Tidak Ada Sparepart</p>
                                    <p class="text-zinc-650 text-xs mt-1">Belum ada sparepart yang ditambahkan atau tidak sesuai pencarian.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
